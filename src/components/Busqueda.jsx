...
const Busqueda = () => {
    ...
    const {buscador, handleBuscador, requerimientos} = useRequerimientos();
    const requerimientosFiltrados = busqueda === '' ? [] : requerimientos.filter(rq => rq.titulo.toLowerCase().includes(busqueda.toLowerCase()))
        ...
        <Transition.Root show={ buscador } as={Fragment} afterLeave={() => setBusqueda('')}>
            <Dialog as="div" className="fixed inset-0 z-10 overflow-y-auto mt-20 p-4 sm:p-20 md:p-20" onClose={handleBuscador}>
                ...
                <Combobox
                    ...
                    onChange={(requerimiento) => (window.location = `/requerimientos/${requerimiento._id}`)}
                >
                    ...
                    {requerimientosFiltrados.length > 0 && (
                        ...
                        {requerimientosFiltrados.map(req => (
                            <Combobox.Option
                                key={req._id}
                                value={req}
                                className={({active}) => classNames('cursor-default select-none px-4 py-2', active && 'bg-green-700 text-white')}
                            >
                                {req.titulo}
                            </Combobox.Option>
                        ))}
